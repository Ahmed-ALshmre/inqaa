...
بعد حفظ الرابط والمفتاح، استخدم زر **اختبار الربط ومزامنة المتاجر**. يجلب النظام
المتاجر المفعلة من Menger ويطابق كل متجر محلي باسمه ويحفظ معرفه منفصلاً. يعرض الزر
أي متجر لم يجد له اسماً مطابقاً بدلاً من توجيه طلبه إلى متجر آخر بالخطأ.
...
## عالم البراءة — ملابس الأطفال

يُنشأ متجر `baraah-kids` تلقائياً بإعدادات وتعليمات مستقلة لملابس الأطفال. ربط Chatwoot الموثق للحساب 184605 هو Facebook inbox 139094 وInstagram inbox 139098؛ تُحفظ هذه الروابط في `app_settings` بمفاتيح `chatwoot_inbox_store:<account>:<inbox>` دون استبدال إعداد سابق. `CHATWOOT_INBOX_STORES` يبقى المرجع الأعلى أولوية إذا احتوى ربطاً صريحاً لنفس الصندوق. يستخدم الرابط العام الحالي `/chatwoot/webhook`، ولا يحتاج إضافة webhook مكرر.

يستخدم المتجر كتالوجه فقط، ولا يرث ملفات تعليمات المنتجات النسائية العامة. يلزم إضافة منتجات الأطفال وصورها ومقاساتها الفعلية في لوحة المتجر قبل عرضها للزبائن. لا ينقل هذا التحديث المحادثات القديمة أو يرسل رسائل تجريبية للعملاء. إعدادات التوصيل الافتراضية الحالية تبقى قابلة للتعديل لكل متجر.
